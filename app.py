...
def get_connection():
    url_db = "postgresql://neondb_owner:npg_p1Wo2mLadQDv@ep-quiet-glade-acuro7hn-pooler.sa-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"
...
def login():
    if request.method == "POST":
        ...
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id_usuario, nombre, clave, rol FROM tbl_usuarios WHERE usuario = %s", (usuario,))
...
def picking():
    ...
    if request.method == "POST":
        ...
        cursor.execute("UPDATE tbl_ubicaciones SET estado = 'Libre' WHERE id_ubicacion IN (SELECT id_ubicacion FROM tbl_pallet_ubicacion WHERE id_pallet = %s AND vigente = TRUE)", (request.form["id_pallet"],))
        conn.commit()
        conn.close()
        return redirect(url_for("dashboard"))
